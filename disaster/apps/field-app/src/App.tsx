...
export default function App({ onVoiceInput }: AppProps = {}) {
  ...
  const commitDelta = async () => {
    ...
    const delta: SyncItem = {
      ...
      sync_status: 'PENDING',
      metadata: {
        title: `[UNIT 17] ${pendingIntent.entity_type} ${pendingIntent.entity_id}: ${pendingIntent.new_state}`,
        details: pendingIntent.extracted_params.raw_text,
        raw_text: pendingIntent.extracted_params.raw_text,
        location_description: pendingIntent.extracted_params.location_description,
        severity: pendingIntent.new_state === 'CRITICAL' ? 'CRITICAL' : (pendingIntent.new_state === 'DAMAGED' || pendingIntent.new_state === 'BLOCKED' ? 'HIGH' : 'MEDIUM'),
        category: 'FIELD REPORT',
        suggested_action: pendingIntent.new_state === 'CRITICAL' || pendingIntent.new_state === 'DAMAGED'
          ? 'Immediate tactical intervention recommended. Review access corridors and dispatch units.'
          : 'Monitor status and notify nearby responders.',
        is_alert: true,
      },
    };
