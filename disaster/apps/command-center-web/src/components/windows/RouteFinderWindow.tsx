...
function generateOfflineAreaRoutes(nodes: RoadNode[], areaLabel: string): RouteDetail[] {
  ...
  const s = nodes[2] || { lat: 26.1580, lng: 91.7800, id: 'node-assam-shelter' };
  const h = nodes[3] || { lat: 26.1500, lng: 91.7700, id: 'node-assam-hospital' };
  const w = nodes[4] || { lat: 26.1300, lng: 91.7000, id: 'node-assam-highway' };
...
export default function RouteFinderWindow({
  ...
  const handleCommitChoice = (r: RouteDetail) => {
    ...
    if (onDispatchTask) {
      const targetEntity = areaId === 'assam-demo' ? 'bridge-assam-01' : areaId === 'delhi-demo' ? 'bridge-delhi-01' : 'road-b12';
      onDispatchTask(`Emergency Route ${r.routeId} Dispatch (${r.distanceKm}km, ${r.estimatedTimeMin}m)`, targetEntity);
    }
  };
